...
object A2dpSinkHelper {
    ...
    private val TAG = "A2DPSinkHelper"
     ...
     * receive.
     */
    val ACTION_CONNECTION_STATE_CHANGED =
     ...
     * receive.
     */
    val ACTION_PLAYING_STATE_CHANGED =
    ...
    fun disconnect(profile: BluetoothProfile, device: BluetoothDevice): Boolean {
        try {
            val m = profile::class.java.getMethod("disconnect", BluetoothDevice::class.java)
            m.invoke(profile, device)
            return true
        } catch (e: NoSuchMethodException) {
            Log.w(
                TAG, "No disconnect method in the " + profile::class.simpleName +
                        " class, ignoring request."
            )
            return false
        } catch (e: InvocationTargetException) {
            Log.w(
                TAG, "Could not execute method 'disconnect' in profile " +
                        profile::class.simpleName + ", ignoring request.", e
            )
            return false
        } catch (e: IllegalAccessException) {
            Log.w(
                TAG,
                "Could not execute method 'disconnect' in profile " + profile::class.simpleName + ", ignoring request.",
                e
            )
            return false
        }
