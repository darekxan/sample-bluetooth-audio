 ...
 * or press a button attached to the GPIO pin returned by [BoardDefaults.getGPIOForPairing]
 *
 ...
 * returned by [BoardDefaults.getGPIOForDisconnectAllBTDevices]
 *
 *
...
class A2dpSinkActivity : Activity() {
    ...
    private val mSinkProfileStateChangeReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.action == A2dpSinkHelper.ACTION_CONNECTION_STATE_CHANGED) {
                ...
                Log.d(
                    TAG, "Bluetooth A2DP sink changing connection state from " + oldState +
    ...
    private val mSinkProfilePlaybackChangeReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.action == A2dpSinkHelper.ACTION_PLAYING_STATE_CHANGED) {
                ...
                val device = A2dpSinkHelper.getDevice(intent)
                Log.d(
                    TAG, "Bluetooth A2DP sink changing playback state from " + oldState +
                            " to " + newState + " device " + device
                )
                if (device != null) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        registerReceiver(
            mAdapterStateChangeReceiver, IntentFilter(
                BluetoothAdapter.ACTION_STATE_CHANGED
            )
        )
        registerReceiver(
            mSinkProfileStateChangeReceiver, IntentFilter(
                A2dpSinkHelper.ACTION_CONNECTION_STATE_CHANGED
            )
        )
        registerReceiver(
            mSinkProfilePlaybackChangeReceiver, IntentFilter(
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE_ENABLE_DISCOVERABLE) {
            ...
            if (resultCode == Activity.RESULT_CANCELED) {
                Log.e(
                    TAG,
                    "Enable discoverable has been cancelled by the user. " + "This should never happen in an Android Things device."
                )
                return
            }
            Log.i(
                TAG, "Bluetooth adapter successfully set to discoverable mode. " +
            ...
            speak(
                "Bluetooth audio sink is discoverable for " + DISCOVERABLE_TIMEOUT_MS +
    ...
    private fun configureButton() {
        try {
            mPairingButtonDriver = ButtonInputDriver(
                BoardDefaults.gpioForPairing,
            ...
            Log.w(
                TAG,
                "Could not register GPIO button drivers. Use keyboard events to trigger " + "the functions instead",
                e
            )
        }
        ...
        mTtsEngine = TextToSpeech(this@A2dpSinkActivity,
            TextToSpeech.OnInitListener { status ->
                if (status == TextToSpeech.SUCCESS) {
                    ...
                    Log.w(
                        TAG, "Could not open TTS Engine (onInit status=" + status
    ...
    companion object {
        private val TAG = "A2dpSinkActivity"

        private val ADAPTER_FRIENDLY_NAME = "My Android Things device"
        private val DISCOVERABLE_TIMEOUT_MS = 300
        private val REQUEST_CODE_ENABLE_DISCOVERABLE = 100

        private val UTTERANCE_ID = "com.example.androidthings.bluetooth.audio.UTTERANCE_ID"
    }
}
